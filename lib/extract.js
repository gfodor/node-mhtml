...
MHTMLExtractor.prototype = {
          ...
          if (meta) {
            if (meta[1].toLowerCase() == "content-location") {
              ...
              if (path.length > 128) {
                ...
                path = _.map(path.split("/"), function(piece) {
                  return piece.length > 64 ? hat() : piece;
                }).join("/");
              }

              part.meta("content-location", location);
              part.meta("content-local-path", path);
            } else {
  ...
  extract: function (dest, cb, force, includeExternal, includeMeta) {
...
function extractMHTML(source, destination, cb, force, includeExternal, includeMeta) {
  new MHTMLExtractor(source).extract(destination, cb, force, includeExternal, includeMeta);
}
