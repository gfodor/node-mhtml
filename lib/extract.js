...
MHTMLExtractor.prototype = {
  ...
  extractParts: function (dest, cb, force, includeExternal) {
  ...
  extract: function (dest, cb, force, includeExternal) {
    var force = (typeof force === 'undefined') ? false : force;
    var force = (typeof includeExternal === 'undefined') ? false : includeExternal;
    var self = this;

    this.getParts(function readFile(err) {
      if (err) return cb(err);
      this.extractParts(dest, function extractParts(err) {
        cb(err, self.primaryPart.extractedToPath);
      }, force, includeExternal);
    }.bind(this));
  }
};

function extractMHTML(source, destination, cb, force, includeExternal) {
  new MHTMLExtractor(source).extract(destination, cb, force, includeExternal);
}
