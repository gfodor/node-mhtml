...
Part.prototype = {
  ...
  filePath: function () {
    ...
    if (this.meta('content-local-path')) {
      return this.meta('content-local-path').replace(/\\/g, '/').replace(this.basePath, '');
    }
  ...
  contentLocationPrefix: function() {
    var location = this.meta('content-location')

    if (!location) {
      return null;
    } else {
      var locationUrl = url.parse(location.toLowerCase());

      if (locationUrl.protocol == "file:" || !locationUrl.host) {
        return locationUrl.protocol;
      } else {
        var hostParts = locationUrl.host.split(".");
  ...
  hasContentLocationPrefix: function(prefix) {
    if (!this.meta('content-location') || !prefix) {
      return true;
    } else {
      return this.contentLocationPrefix() == prefix;
    }
  },
